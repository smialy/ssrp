...
SRPClient = function (username, password, group, hashFn) {
  ...
  var toHash = this.N.toString() +':'+this.g.toString();
  this.k = new BigInteger(this.hash(toHash), 16);
...
SRPClient.prototype = {
  ...
  calculateU: function(A, B) {
    ...
    return new BigInteger(this.hash(A.toString()+':'+B.toString()), 16);
  ...
  calculateM: function (A, B, K) {
    ...
    if (A.mod(this.N).toString() == '0' ||
        B.mod(this.N).toString() == '0')
      throw 'ABORT: illegal_parameter';
    return new BigInteger(this.hash(A.toString() +':'+ B.toString()+':'+K.toString()), 16);
  ...
  calculateS: function(B, salt, uu, aa) {
    ...
    //(B - kg^x) ^ (a + ux)
    var S = B.subtract((this.k.multiply(bx))).modPow(aa.add(uu.multiply(x)),this.N)
    return S;
  ...
  calculateK: function (S) {
    return new BigInteger(this.hash(S.toString()), 16);
  },
